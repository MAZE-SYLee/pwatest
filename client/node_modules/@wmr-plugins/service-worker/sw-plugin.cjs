'use strict';const path=require('path'),http=require('http');const pathToPosix = p => p.split(path.sep).join(path.posix.sep);
...
function swPlugin(options) {
		...
		load(id) {
			if (id.startsWith('/@npm/'))
				return new Promise((y, n) => {
					http.request({ ...loopback, path: id }, res => {
						let data = '';
	...
	options.plugins.push({
		...
		async load(id) {
			...
			try {
				var { rollup } = await Promise.resolve().then(function(){return require('rollup')});
			} catch (e) {
